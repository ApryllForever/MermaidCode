...
namespace RestStopCode
{
    ...
    public static class TouchActionProperties
    {
        ...
        private static void getEpicSword()
        {
            Game1.flashAlpha = 1f;
            Game1.player.addItemByMenuIfNecessaryElseHoldUp(new MeleeWeapon("62"));
            Game1.player.reduceActiveItemByOne();
            if (!Game1.player.addItemToInventoryBool(new MeleeWeapon("62")))
            {
                Game1.createItemDebris(new MeleeWeapon("62"), Game1.player.getStandingPosition(), 1);
            }
            Game1.player.mailReceived.Add("11429EpicSword");
            Game1.player.jitterStrength = 0f;
            Game1.screenGlowHold = false;

        }
        private static void getEpicHammer()
        {
            Game1.flashAlpha = 1f;
            Game1.player.addItemByMenuIfNecessaryElseHoldUp(new MeleeWeapon("63"));
            Game1.player.reduceActiveItemByOne();
            if (!Game1.player.addItemToInventoryBool(new MeleeWeapon("63")))
            {
                Game1.createItemDebris(new MeleeWeapon("63"), Game1.player.getStandingPosition(), 1);
            }
            Game1.player.mailReceived.Add("11429EpicHammer");
            Game1.player.jitterStrength = 0f;
            Game1.screenGlowHold = false;

        }
        private static void getEpicDagger()
        {
            Game1.flashAlpha = 1f;
            Game1.player.addItemByMenuIfNecessaryElseHoldUp(new MeleeWeapon("64"));
            Game1.player.reduceActiveItemByOne();
            if (!Game1.player.addItemToInventoryBool(new MeleeWeapon("64")))
            {
                Game1.createItemDebris(new MeleeWeapon("64"), Game1.player.getStandingPosition(), 1);
            }
            Game1.player.mailReceived.Add("11429EpicDagger");
            Game1.player.jitterStrength = 0f;
            Game1.screenGlowHold = false;
            ...
            try
            {
                ...
                switch (fields[0].ToLower()) //check the TouchAction's name in lowercase
                {
                    ...
                    case "reststop_heal":
                        ...
                        Game1.changeMusicTrack("none", track_interruptable: false, StardewValley.GameData.MusicContext.Event);
                    ...
                    case "reststop_message": //TouchAction Messages
                        ...
                        break;
                    case "epicsword":
                        // if (Game1.player.mailReceived.Contains("11429SmileofMorana"))
                        {
                            if (Game1.player.ActiveObject != null && Utility.IsNormalObjectAtParentSheetIndex(Game1.player.ActiveObject, "(O)577") && !Game1.player.mailReceived.Contains("11429EpicSword"))
                            {
                                Game1.player.Halt();
                                Game1.player.faceDirection(2);
                                Game1.player.showCarrying();
                                Game1.player.jitterStrength = 1f;
                                Game1.pauseThenDoFunction(7000, getEpicSword);
                                Game1.changeMusicTrack("none", track_interruptable: false, StardewValley.GameData.MusicContext.Event);
                                Game1.playSound("crit");
                                Game1.screenGlowOnce(new Color(111, 0, 170), hold: true, 0.01f, 0.999f);
                            ...
                            else if (!Game1.player.mailReceived.Contains("11429EpicSword"))
                            {
                        ...
                        //else
                        //{
                        //  string MoranaMission = "You feel inside you a dark, cold voice: Smite mine enemies. 'Return my star unto Her place. I will reward you. Blood and Ice be with you. Wind and Grave will guide you. Breathe and Life will save you. Bring then to this place the jewels of my desire.'";
                        // localSound("thunder");
                        //Game1.drawDialogueNoTyping(MoranaMission);
                        //}
                        break;
                    ...
                    case "epichammer":
                        ...
                        //if (Game1.player.mailReceived.Contains("11429SmileofMorana"))
                        //{
                        if (Game1.player.ActiveObject != null && Utility.IsNormalObjectAtParentSheetIndex(Game1.player.ActiveObject, "(O)577") && !Game1.player.mailReceived.Contains("11429EpicHammer"))
                        {
                            Game1.player.Halt();
                            Game1.player.faceDirection(2);
                            Game1.player.showCarrying();
                            Game1.player.jitterStrength = 1f;
                            Game1.pauseThenDoFunction(7000, getEpicHammer);
                            Game1.changeMusicTrack("none", track_interruptable: false, StardewValley.GameData.MusicContext.Event);
                            Game1.playSound("crit");
                            Game1.screenGlowOnce(new Color(30, 0, 200), hold: true, 0.01f, 0.999f);
                            DelayedAction.playSoundAfterDelay("stardrop", 1500);
                            Game1.screenOverlayTempSprites.AddRange(Utility.sparkleWithinArea(new Microsoft.Xna.Framework.Rectangle(0, 0, Game1.viewport.Width, Game1.viewport.Height), 500, Color.White, 10, 2000));
                            Game1.afterDialogues = (Game1.afterFadeFunction)Delegate.Combine(Game1.afterDialogues, (Game1.afterFadeFunction)delegate
                            {
                        ...
                        else if (!Game1.player.mailReceived.Contains("11429EpicHammer"))
                        {
                            Game1.playSound("SpringBirds");
                        }
                        //}
                    ...
                    case "epicdagger":
                        // if (Game1.player.mailReceived.Contains("11429SmileofMorana"))
                        // {
                        if (Game1.player.ActiveObject != null && Utility.IsNormalObjectAtParentSheetIndex(Game1.player.ActiveObject, "(O)577") && !Game1.player.mailReceived.Contains("11429EpicDagger"))
                        {
                            Game1.player.Halt();
                            Game1.player.faceDirection(2);
                            Game1.player.showCarrying();
                            ...
                            Game1.changeMusicTrack("none", track_interruptable: false, StardewValley.GameData.MusicContext.Event);
                            Game1.playSound("crit");
                            Game1.screenGlowOnce(new Color(200, 0, 200), hold: true, 0.01f, 0.999f);
                            DelayedAction.playSoundAfterDelay("stardrop", 1500);
                            Game1.screenOverlayTempSprites.AddRange(Utility.sparkleWithinArea(new Microsoft.Xna.Framework.Rectangle(0, 0, Game1.viewport.Width, Game1.viewport.Height), 500, Color.White, 10, 2000));
                            Game1.afterDialogues = (Game1.afterFadeFunction)Delegate.Combine(Game1.afterDialogues, (Game1.afterFadeFunction)delegate
                            {
                        ...
                        else if (!Game1.player.mailReceived.Contains("11429EpicDagger"))
                        {
                            Game1.playSound("SpringBirds");
                        }
                        //}
        ...
        private static void OnUpdateTicked(object sender, UpdateTickedEventArgs e)
        {
            ...
            Vector2 tile = Game1.player.Position; //get the current local player's tile
