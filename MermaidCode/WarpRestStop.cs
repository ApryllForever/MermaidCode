...
{
    internal static class WarpTotem2
    {
        static readonly string Destination = "custom_JunkBeach";
        static readonly int Dest_X = 64;
        static readonly int Dest_Y = 22;
        ...
        {
            Totem = "(O)ApryllForever.RiseMermaids_LostTownTotem";
        }
        ...
        private static void OnButtonPressed(object sender, ButtonPressedEventArgs e)
        {
            ...
            if (e.Button.IsActionButton())
            {
                try
                {
                    if (Game1.player.CurrentItem?.Name != null)
                    {
                        ...
                        if ((bool)(Game1.player.CurrentItem?.Name.Contains("Lost Town")))
                        {
                            Monitor.Log($"RestStop: Using Warp Totem: LostTown");
            ...
            if (e.Button.IsActionButton())
            {
                try
                {
                    ...
                    {
                        Monitor.Log($"RestStop: Using Warp Totem: Lost Town");
                        Game1.player.reduceActiveItemByOne();
                        DoTotemWarpEffects(Game1.player, (f) => DirectWarp());
                    }
                }
        ...
        private static void DoTotemWarpEffects(Farmer who, Func<Farmer, bool> action)
        {
            ...
            who.currentLocation.playSound("warrior", null, null, StardewValley.Audio.SoundContext.Default);
            who.faceDirection(2);
            who.canMove = false;
