...
namespace RestStopLocations.Quests
{
	internal class MermaidSpecialOrderBoard : SpecialOrdersBoard
	{
		...
		internal MermaidSpecialOrderBoard(string boardType = "") : base(boardType)
		{
			...
			Texture2D texture;
			if (Game1.currentLocation.Equals("Custom_RestStop"))
			{
				boardType.Equals(MermaidConstants.Z_RANGERSPECIALORDER);
				texture = Game1.temporaryContent.Load<Texture2D>("LooseSprites\\RangerSOBoard");

			}
			else if (Game1.currentLocation.Equals("Custom_SapphireSprings"))
			{
				boardType.Equals(MermaidConstants.Z_SAPPHIRESPECIALORDER);
		...
		public static void UpdateAvailableMermaidSpecialOrders(bool force_refresh)
		{
			if (Game1.player.team.availableSpecialOrders is not null)
			{
				...
				{
					if ((order.questDuration.Value == StardewValley.GameData.SpecialOrders.QuestDuration.TwoDays || order.questDuration.Value == StardewValley.GameData.SpecialOrders.QuestDuration.ThreeDays) && !Game1.player.team.acceptedSpecialOrderTypes.Contains(order.orderType.Value))
					{
			...
			for (int k = 0; k < keys.Count; k++)
			{
				...
				if (Game1.dayOfMonth >= 16.Equals(order_data[key].Duration == "Month"))
				{
